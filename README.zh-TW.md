...
- 涵蓋登入、金幣大廳、選場、活動入口與牌桌的行動端產品流程。
- 展示金幣場、私人局、俱樂部、SNG、MTT 與多玩法入口設計。
- 使用 Unity C# 與 Lua UI Adapter，便於大廳列表及活動頁面擴充。
- 提供 C++ 登入、登出、用戶資料和伺服器映射非同步回呼參考。
- 提供真實登入、大廳活動和 SNG 選場截圖。

## 核心功能與核心特色

| 模組 | 功能說明 |
|---|---|
| 帳號登入 | 手機號登入、用戶狀態、非同步登入與登出回呼 |
| 金幣大廳 | 多級別選場、買入檔位、快速入桌和營運活動入口 |
| 經典德州 | 下注、跟注、加注、棄牌、All-in 和結算流程 |
| 私人局與俱樂部 | 好友組局、俱樂部牌桌、成員和戰績入口 |
| SNG 與 MTT | 單桌賽、多桌錦標賽、報名和選場介面 |
| 玩法擴充 | 產品資料展示短牌、奧馬哈等擴充方向 |
| Unity/Lua UI | 列表、表格與通用 UI 物件橋接 |
| C++ 回呼 | 登入、用戶資料、狀態和伺服器映射回呼 |

以上是產品展示範圍；公開程式碼覆蓋程度請以「公開原始碼與資料」及 `BUILD-SCOPE.md` 為準。
