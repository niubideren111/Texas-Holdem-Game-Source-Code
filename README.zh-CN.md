...
- 覆盖登录、金币大厅、选场、活动入口与牌桌的移动端产品流程。
- 展示金币场、私人局、俱乐部、SNG、MTT 及多玩法入口设计。
- 使用 Unity C# 与 Lua UI Adapter，便于大厅列表和活动页面扩展。
- 提供 C++ 登录、登出、用户信息和服务器映射异步回调参考。
- 提供真实登录、大厅活动和 SNG 选场截图。

## 核心功能与核心特色

| 模块 | 功能说明 |
|---|---|
| 账号登录 | 手机号登录、用户状态、异步登录和登出回调 |
| 金币大厅 | 多级别选场、买入档位、快速入桌和运营活动入口 |
| 经典德州 | 下注、跟注、加注、弃牌、All-in 和结算流程 |
| 私人局与俱乐部 | 好友组局、俱乐部牌桌、成员和战绩入口 |
| SNG 与 MTT | 单桌赛、多桌锦标赛、报名和选场界面 |
| 玩法扩展 | 产品资料展示短牌、奥马哈等扩展方向 |
| Unity/Lua UI | 列表、表格与通用 UI 对象桥接 |
| C++ 回调 | 登录、用户数据、状态和服务器映射回调 |

以上是产品展示范围；公开代码覆盖程度请以“公开源码与资料”及 `BUILD-SCOPE.md` 为准。
